...
/// Generate entire statistics map from
      ...
      if (game.teamId != null && game.startTime != null && !game.gameActions.isEmpty && game.id != null && game.stopWatchTimer != null) {
        ...
        if (game.stopTime != null) {
          ...
          DateTime stopWatchTimeAsDateTime =
              DateTime.fromMillisecondsSinceEpoch((DateTime.fromMillisecondsSinceEpoch(0).millisecond + game.stopWatchTimer!.rawTime.value).toInt());
          ...
          stopTimeAsIsoTimeStamp = DateTime.fromMillisecondsSinceEpoch(game.startTime!).add(Duration(minutes: 60)).millisecondsSinceEpoch;
        }
  ...
  Map<String, dynamic> _generatePlayerStatistics(List<GameAction> actions, List<Player> players) {
    ...
    Map<String, int> updateActionCounts(GameAction action, Map<String, int> action_counts) {
    ...
    ///
    /// Called for every action.
    Map<String, List<int>> updateActionSeries(GameAction action, Map<String, List<int>> action_series) {
    ...
    ///
    /// Called for every action
    Map<String, dynamic> updateActionCoordinates(GameAction action, Map<String, dynamic> action_coordinates) {
    ...
    List<List<double>> updateQuotas(GameAction action, List<List<double>> quotas) {
      switch (action.tag) {
